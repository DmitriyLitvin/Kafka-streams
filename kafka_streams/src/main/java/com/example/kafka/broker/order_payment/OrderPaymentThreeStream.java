...
public class OrderPaymentThreeStream {
    ...
    public KStream<String, OnlineOrderMessage> kStreamOrderPayment(StreamsBuilder streamsBuilder) {
        ...
        orderStream.outerJoin(paymentStream, this::joinerOrderPayment, JoinWindows.of(Duration.ofHours(1)),
                StreamJoined.with(stringSerde, orderSerde, paymentSerde))
                .through("t.commodity.join-order-payment-three", Produced.with(stringSerde, orderPaymentSerde))
    ...
    private OnlineOrderPaymentMessage joinerOrderPayment(OnlineOrderMessage orderMessage,
        ...
        if (orderMessage != null) {
            orderPaymentMessage.setOnlineOrderNumber(orderMessage.getOnlineOrderNumber());
