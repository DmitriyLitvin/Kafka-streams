package com.example.kafka.broker.vote;

import com.example.kafka.broker.message.WebColorVoteMessage;
import com.example.kafka.broker.message.WebDesignVoteMessage;
import com.example.kafka.broker.message.WebLayoutVoteMessage;
import com.example.kafka.util.WebColourVoteTimestampExtractor;
import com.example.kafka.util.WebLayoutVoteTimestampExtractor;
...
public class WebDesignVoteOneStream {
    ...
    public KStream<String, WebDesignVoteMessage> kstreamWebDesignVote(StreamsBuilder builder) {
        ...
        builder.stream("t.commodity.web.vote-color", Consumed.with(stringSerde, colorSerde, new WebColourVoteTimestampExtractor(), null))
                .mapValues(v -> v.getColor()).to("t.commodity.web.vote-one-username-color");

        var colorTable = builder.table("t.commodity.web.vote-one-username-color", Consumed.with(stringSerde, stringSerde));
        ...
        var layoutTable = builder.table("t.commodity.web.vote-one-username-layout", Consumed.with(stringSerde, stringSerde));

        var joinTable = colorTable.join(layoutTable, this::voteJoiner, Materialized.with(stringSerde, designSerde));
        joinTable.toStream().to("t.commodity.web.vote-one-stream");
