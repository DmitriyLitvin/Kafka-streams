...
public class FeedBackTwoStream {
    ...
    public KStream<String, String> kstreamFeedBack(StreamsBuilder builder) {
        ...
        KStream<String, Long> goodFeedback = feedbackStream[0].groupByKey().count().toStream();
        goodFeedback.print(Printed.<String, Long>toSysOut().withLabel("good feed"));
        goodFeedback.to("t.commodity.feedback.good-word");
        KStream<String, Long> badFeedback = feedbackStream[1].groupByKey().count().toStream();
        badFeedback.print(Printed.<String, Long>toSysOut().withLabel("bad feed"));
        badFeedback.to("t.commodity.feedback.bad-word");
